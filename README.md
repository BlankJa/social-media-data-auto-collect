...
> **状态：第一期交付 + 多账号规模化已落地。** 四平台（B 站 / 微博 / 抖音 / 快手）全链路打通并经真机验证，登录扫码、增量采集、Excel 渲染、status 健康面板、跨平台调度模板均已实现；`collect all` 已升级为四平台并行采集，status 面板新增账号成功率与失败账号清单。63 个测试全过。进度详见 [docs/superpowers/plans/](docs/superpowers/plans/)。
...
# 4. 跑采集
uv run python cli.py collect all              # 四平台并行增量（平台内串行+节流）
...
```
                       新媒体采集状态
┏━━━━━━━━━━┳━━━━━━━━━━━━━┳━━━━━━━┳━━━━━━━━┳━━━━━━━━━┓
┃ 平台     ┃ Cookie 年龄 ┃ 近7天 ┃ 账号   ┃ 建议    ┃
┡━━━━━━━━━━╇━━━━━━━━━━━━━╇━━━━━━━╇━━━━━━━━╇━━━━━━━━━┩
│ bilibili │ 3天         │ 12    │ 3/3 ✓  │ ✅ 正常 │
│ kuaishou │ 6天         │ 0     │ 1/2 ✓  │ ⚠️ 快过期，建议重扫 │
│ weibo    │ —           │ 0     │ —      │ ❌ 未登录，需扫码   │
└──────────┴─────────────┴───────┴────────┴─────────┘
❌ 失败账号：
  kuaishou / 某某大学 (3xabc)：cookie 失效
重扫命令：cli.py login <平台>
```

- **Cookie 年龄**实时从 cookie 文件算（不依赖上次运行），所以没跑过采集也能看出谁该重扫。**快手是 7 天免登录会话，约每周过期一次**，满 6 天即提示；其余平台 25 天阈值。
- **账号**列显示上次采集的 `成功/总数`，表下列出具体失败的账号（平台 / 名称 / id / 原因），定位某个账号失效不用翻日志。
...
当前 63 个测试覆盖 `schemas` / `cookies` / `storage` / `base` / `accounts` / 四平台 parser / 列定义 / 渲染选取 / status / 并行采集，全过。

### 实施路线

**第一期：四平台流水线**（`docs/superpowers/plans/2026-06-08-新媒体采集流水线.md`，已完成）
...
**第二期：采集体验提升**（`docs/superpowers/plans/2026-06-16-采集体验提升.md`，已完成）

- status 改为实时面板（cookie 年龄 + 建议）、`account` 子命令族（增删查 / 批量导入）✅

**第三期：多账号规模化**（`docs/superpowers/plans/2026-06-17-多账号规模化并行采集.md`，已完成）

- `collect all` 跨平台并行（ThreadPoolExecutor）、失败账号可见性（status 面板账号列 + 失败清单）✅
